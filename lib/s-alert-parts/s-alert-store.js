...
const update = (state, action) => {
    let elemToUpdateArray = state.slice().filter(item => item.id === action.data.id);
    if (Array.isArray(elemToUpdateArray)) {
        let elemToUpdateIndex = state.indexOf(elemToUpdateArray[0]);
        state[elemToUpdateIndex] = action.data;
        return [ ...state ];
    }
    return state;
};
